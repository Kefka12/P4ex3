...
def writeTunnelRules(p4info_helper, ingress_sw, egress_sw, tunnel_id,
    ...
    table_entry = p4info_helper.buildTableEntry(
        ...
        action_params={
            "port": 2 #换到切换端口，与出口转发规则存在区别
        })
    ingress_sw.WriteTableEntry(table_entry)#在入口转发规则中写入该表内容
...
if __name__ == '__main__':
    ...
    parser.add_argument('--p4info', help='p4info proto in text format from p4c',
                        ...
                        default='./build/ecn_at_master.p4.p4info.txt')
    parser.add_argument('--bmv2-json', help='BMv2 JSON file from p4c',
                        type=str, action="store", required=False,
                        default='./build/ecn_at_master.json')
    
    #mri实验的规则引入
    #parser.add_argument('--p4info', help='p4info proto in text format from p4c',
    #                    type=str, action="store", required=False,
    #                   default='./build/mri_at_master.p4.p4info.txt')
    #parser.add_argument('--bmv2-json', help='BMv2 JSON file from p4c',
    #                   type=str, action="store", required=False,
    #                    default='./build/mri_at_master.json')
    #
